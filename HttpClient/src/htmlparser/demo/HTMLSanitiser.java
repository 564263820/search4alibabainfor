 ...
 * <code>&lt;li&gt;</code> elements must be inside <code>&lt;ul&gt;</code>
 * or <code>&lt;ol&gt;</code> elements). If not, reject the element.
 * <li>Keep only the allowed attributes (<code>id</code>,
 * <code>class</code>, <code>href</code>, <code>target</code>,
 * <code>title</code>) and strip any others.
 ...
 * <ul>
 * <li>If the method strips invalid markup, completely remove the tag or
 * element from the output, otherwise encode it so that it renders verbatim.
 * </ul>
 * </li>
 * </ul>
 * </li>
 * <li> If the <code>formatWhiteSpace</code> option is enabled:
 * <ul>
 * <li>Line breaks, being Carriage Return (U+000D) or Line Feed (U+000A)
 * characters, and Form Feed characters (U+000C) are converted to "<code>&lt;br /&gt;</code>".
 * CR/LF pairs are treated as a single line break.
 * <li>Multiple consecutive spaces are converted so that every second space is
 * converted to "<code>&amp;nbsp;</code>" while ensuring the last is always
 * a normal space.
...
public class HTMLSanitiser {
	 ...
	 * <pre style="margin:0">
	 * HTMLSanitiser
	 * 		.encodeInvalidMarkup(&quot;&lt;P&gt;&lt;u&gt;Line   1&lt;/u&gt;\n&lt;b&gt;Line   2&lt;/b&gt;\n&lt;script&gt;doBadStuff()&lt;/script&gt;&quot;)
	 * </pre>
	 * 
	 * </td>
	 * </tr>
	 ...
	 * <li>The unwanted <code>&lt;u&gt;</code> and
	 ...
	 * elements
	 * <li>Non-breaking spaces (<code>&amp;nbsp;</code>) are added to
	 * ensure the multiple spaces are rendered as they appear in the input.
	 * </ul>
	 * </dd>
	 * </dl>
	 * 
	 ...
	 * <pre style="margin:0">
	 * HTMLSanitiser
	 * 		.stripInvalidMarkup(&quot;&lt;P&gt;&lt;u&gt;Line   1&lt;/u&gt;\n&lt;b&gt;Line   2&lt;/b&gt;\n&lt;script&gt;doBadStuff()&lt;/script&gt;&quot;)
	 * </pre>
	 * 
	 * </td>
	 * </tr>
	 ...
	 * <li>The <code>&lt;b&gt;</code> element is kept
	 * <li>The unwanted <code>&lt;u&gt;</code> and
	 * <code>&lt;script&gt;</code> elements are stripped from the output
	 * </ul>
	 * </dd>
	 * </dl>
	 * 
		...
		if (tag.getTagType() == StartTagType.NORMAL) {
			...
			if (HTMLElements.getEndTagRequiredElementNames().contains(
				...
				if (element.getEndTag() == null)
					return false; // refect start tag if its required end tag
									// is missing
				...
				if (element.getEndTag() == null)
					outputDocument.insert(element.getEnd(),
							getEndTagHTML(elementName)); // insert optional
															// end tag if it is
															// missing
			}
	...
	private static boolean isValidLITag(Tag tag) {
		...
		return parentElement.getName() == HTMLElementName.UL
				|| parentElement.getName() == HTMLElementName.OL; // only
																	// accept LI
																	// tags
																	// who's
																	// immediate
