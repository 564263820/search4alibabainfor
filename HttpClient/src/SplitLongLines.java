...
public class SplitLongLines {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			reader = new BufferedReader(new InputStreamReader(sourceUrl
					.openStream()));
			String line;
			while ((line = reader.readLine()) != null) {
				...
				for (Tag tag : source.getAllTags()) {
					if (pos != tag.getBegin())
						print(line.subSequence(pos, tag.getBegin())); // print
																		// the
																		// text
																		// between
																		// this
				...
				if (pos != line.length())
					print(line.subSequence(pos, line.length())); // print the
																	// text
																	// between
																	// the last
																	// tag and
