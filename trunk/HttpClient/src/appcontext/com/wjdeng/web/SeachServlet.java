...
public class SeachServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if ("retry".equals(operation)) {
			...
			try {
				runTask(url, 0,request.getRequestedSessionId());
			} catch (Exception e) {
	...
	private void runTask(String url,int deep,final String sessionId) throws Exception{
		AppContext app;
		ModeParament  par=map.get(sessionId);
		if (par == null) {
		...
		app.addListener4AfterNextPage(new Listener(){
			...
			public void execute(Event ev) {
				...
				state.setMsg("分页完成！");
				state.setState(AppStatus.running.name());
				state.setData(ev.getModeParament().getDatatemp());
				msg.setTypeKey("paserPage");
				msgService.sendMsg2SessionId(msg,this.session);
			}});
		...
		app.addListener4End(new Listener(){
			...
			public void execute(Event ev) {
				...
				State state =new State();
				state.setMsg("完成！");
				state.setState(AppStatus.end.name());
				state.setData(ev.getModeParament().getDatatemp());
				msg.setTypeKey("paserPage");
				msgService.sendMsg2SessionId(msg,this.session);
