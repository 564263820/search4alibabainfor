 ...
 * File Name       : GllobalSourcesPaser.java
 *
 ********************************************************************************/
...
public class GllobalSourcesPaser implements IPaser {
	...
	public Map<String, String> execuPaseInforPage(Document doc,AppContext appContext) {
		...
		if (!list.isEmpty()) {
			...
			int start = doc.getSource().getPreviousStartTag(
					eltem.getContent().getBegin()).getPreviousTag()
					.getPreviousTag().getPreviousTag().getBegin();
			Iterator<Element> it = doc.getSource().getNextElement(start)
					.getAllElements("p").iterator();// .get(0).getTextExtractor().toString();
			while (it.hasNext()) {
				Element ele = it.next();
				...
				for (Element strele : stronglist) {
					if (null == strele.getContent().getFirstElement())
						continue;
					String name = strele.getContent().getFirstElement()
							.getName();
					if ("b".equals(name)) {
						String cname = strele.getContent().getFirstElement()
								.getTextExtractor().toString();
						contentmap.put("Company Name:", cname);
						contentStr = contentStr.substring(contentStr
								.indexOf(cname)
								+ cname.length());
				...
				if (contentStr.indexOf("Key Contact:") > -1) {
					String contactPerson = contentStr.substring(contentStr
							.indexOf("Key Contact:")
							+ "Key Contact:".length());
	...
	private void setCotent(String contentStr, Map<String, String> contentmap) {
		...
		for (String item : items) {
			...
			if (carray.length > 1) {
				lastKey = carray[0];
				if ("http".equals(StringUtils.trim2empty(lastKey)))
					continue;
	...
	public Set<String> getPageListUrl(Document doc,AppContext appContext) {
		...
		for (Element eleTr : eleTrs) {
			// String reg = "(*){1,}";
			Pattern pa = Pattern.compile("[^\"\']{1,}");// eleTr.getAllElements("title",
														// pa);
	...
	public static void main(String[] s) {
		...
		// String
		// str="http://us.my.alibaba.com/userbehavior/contactPersonUpdate.htm?memberId=f%2B63TPzd45lTE0EYPTxtqblHad5z3Es60Nu%2Fg37wnsyBBpp%2Bh9FqSA%3D%3D";
		...
		try {
			File file = new File(GllobalSourcesPaser.class.getClassLoader()
					.getResource("test.txt").getPath());
			java.io.InputStreamReader isr = new InputStreamReader(
			...
			/*
			 * URLContentManage um= new URLContentManage(); Map<String,Object>
			 * map = um.getContentByURL(url,true); Document
			 * doc=AppContext.getHtmlDocByUrl((String) map.get("KEY_CONTENT"));
			 */
