...
var pt = {isHttps:false, err_m:null, init:function () {
	try {
		pt.isHttps = ((window.parent.location + "").substring(0, 5).toLowerCase() == "https");
	}
	catch (A) {
	}
	sys.onload(function () {
...
function ptui_checkVC(A, B) {
	...
	if (defaultuin != "" && g_changeNum <= 1) {
		...
		if (defaultuin != "") {
			C["16"] = g_time.time6 - g_time.time3, C["17"] = g_time.time7 - g_time.time3;
		}
		if (!xuiFrame) {
			ptui_speedReport(C);
		}
	...
	if (C == 0) {
		...
		if (A == 0) {
			if (F && F != "") {
				...
				pt.show_err("??\xa8???\xe8????????\xe8?????\xe8?\xb7\xe9??\xe8?????");
			}
		} else {
			pt.show_err(F);
			$("p").value = "";
...
var stat_list = ["im.qq.com???", "http://ui.ptlogin2.qq.com/", "http://xui.ptlogin2.qq.com/", "http://emailreg.qq.com/", "http://cr.sec.qq.com/", "http://imgcache.qq.com/", "http://gtimg.cn/", "http://gtimg.com/", "http://static.paipaiimg.com/", "http://cache.idqqimg.com"];
function stat(A) {
...
function static_rpt(H) {
	...
	try {
		if (D.length > 0) {
			...
			var E = D.join("|");
			var I;
			var J = "http://cr.sec.qq.com/cr?id=5&d=" + I + "=v" + secVer + "|" + E;
			var G = new Image();
			G.src = J;
		}
	}
